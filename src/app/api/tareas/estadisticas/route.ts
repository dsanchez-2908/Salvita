  ...
  try {
    ...
    const estadisticasGenerales = await query(
      `SELECT 
        ...
        SUM(CASE WHEN FechaVencimiento < GETDATE() AND Estado NOT IN ('Finalizada', 'Rechazada') THEN 1 ELSE 0 END) as Vencidas
       FROM TD_TAREAS t
    ...
    const tiempoPromedio = await query(
      `SELECT 
        AVG(DATEDIFF(DAY, FechaCreacion, FechaFinalizacion)) as PromedioFinalizadas,
        AVG(DATEDIFF(DAY, FechaCreacion, FechaTomada)) as PromedioTomada
       FROM TD_TAREAS t
       WHERE FechaFinalizacion IS NOT NULL ${condicionFechas.replace('t.FechaCreacion', 'FechaCreacion')}`,
      params
    );
    ...
    // 7. Top usuarios que más finalizan tareas
    const topUsuariosFinalizadores = await query(
      `SELECT TOP 10
        u.Id as UsuarioId,
        u.Nombre as UsuarioNombre,
        COUNT(*) as TareasFinalizadas,
        AVG(DATEDIFF(HOUR, t.FechaTomada, t.FechaFinalizacion)) as PromedioHoras
       FROM TD_TAREAS t
       INNER JOIN TD_USUARIOS u ON t.UsuarioTomadaPorId = u.Id
       WHERE t.Estado = 'Finalizada' 
         AND t.FechaTomada IS NOT NULL 
         AND t.FechaFinalizacion IS NOT NULL
         ${condicionFechas}
       GROUP BY u.Id, u.Nombre
       ORDER BY TareasFinalizadas DESC`,
      params
    );
    ...
    const eficienciaTipoAsignacion = await query(
      `SELECT 
        ...
        COUNT(*) as Total,
        SUM(CASE WHEN Estado = 'Finalizada' THEN 1 ELSE 0 END) as Finalizadas,
        CAST(SUM(CASE WHEN Estado = 'Finalizada' THEN 1 ELSE 0 END) * 100.0 / COUNT(*) as DECIMAL(5,2)) as PorcentajeExito
       FROM TD_TAREAS t
       WHERE 1=1 ${condicionFechas}
    ...
    return NextResponse.json<ApiResponse>({
      ...
      data: {
        ...
        topUsuarios: topUsuariosFinalizadores || [],
