...
async function debugTarea5() {
  try {
    await sql.connect(config);
    console.log('Conectado a la base de datos\n');

    // Obtener información de la tarea #5
    ...
    console.log('=== INFORMACIÓN DE TAREA #5 ===');
    if (resultTarea.recordset.length === 0) {
      console.log('❌ No se encontró la tarea #5');
      return;
    }
    ...
    console.log('Usuario Asignado:', tarea.UsuarioAsignadoNombre, `(${tarea.UsuarioAsignadoLogin})`);
    ...
    const resultUsuarios = await sql.query`
    ...
    resultUsuarios.recordset.forEach(u => {
      console.log(`  - ${u.Nombre} (${u.Usuario}) [ID: ${u.Id}]`);
    });
